...
<div class="glass-card-hover group overflow-hidden reveal" data-reveal-delay="{{ ($index % 8) * 50 }}" id="product-card-{{ $product->id }}">
    ...
    <div class="p-5">
        ...
        <div class="flex items-center justify-between">
            ...
            @if ($product->hasVariants())
                <a href="{{ route('products.show', $product) }}" class="p-2 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-white transition-all" title="Chọn biến thể" aria-label="Chọn biến thể cho {{ $product->title }}">
            ...
            @else
                <button type="button" data-cart-add="{{ route('cart.add', $product) }}" class="p-2 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-white transition-all" title="Thêm vào Giỏ hàng" aria-label="Thêm {{ $product->title }} vào giỏ hàng">
