...
@section('content')
<section class="py-12 store-view store-contact-view">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10 reveal">
            <span class="store-section-kicker">Project Brief</span>
            <h1 class="section-heading text-white mt-2">Liên hệ <span class="gradient-text">triển khai</span></h1>
            <p class="text-gray-400 text-lg">Gửi brief cho sản phẩm code, cửa hàng điện tử hoặc hệ thống cần build riêng.</p>
        </div>
        ...
        <div class="glass-card p-8 reveal" data-reveal-delay="100">
            <form action="{{ route('contact.store') }}" method="POST" id="contact-form">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label for="contact-name" class="block text-sm font-medium text-gray-300 mb-2">Tên</label>
                        <input type="text" id="contact-name" name="name" value="{{ old('name') }}" required class="form-input" placeholder="Tên của bạn" autocomplete="name">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="email" id="contact-email" name="email" value="{{ old('email') }}" required class="form-input" placeholder="email@cuaban.com" autocomplete="email">
                        @error('email')
                            <p class="text-danger text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-6">
                    <label for="contact-subject" class="block text-sm font-medium text-gray-300 mb-2">Tiêu đề</label>
                        <input type="text" id="contact-subject" name="subject" value="{{ old('subject') }}" required class="form-input" placeholder="Hỏi đáp dự án, hợp tác, v.v." autocomplete="off">
                    @error('subject')
                        <p class="text-danger text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            <a href="mailto:{{ $contactEmail }}" class="glass-card-hover p-5 text-center reveal block" data-reveal-delay="200">
                ...
                <p class="text-xs text-gray-400 mt-1">{{ $contactEmail }}</p>
            </a>
            <a href="{{ $telegramUrl }}" target="_blank" rel="noopener" class="glass-card-hover p-5 text-center reveal block" data-reveal-delay="250">
            ...
            <a href="{{ $githubUrl }}" target="_blank" rel="noopener" class="glass-card-hover p-5 text-center reveal block" data-reveal-delay="300">
                ...
                <p class="text-xs text-gray-400 mt-1">{{ $githubUrl }}</p>
            </a>
            <div class="glass-card p-5 text-center reveal" data-reveal-delay="350">
